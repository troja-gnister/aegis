...
The current focus is **Phase 2A.1 — Indexed-browser implementation**, starting the indexed-drive milestone. Tasks 1–5 are complete: lossless filename handling, safely scoped verification commands, protected catalog schema, deployment-bound scan state, database-enforced scheduling and leases, and a bounded read-only directory reader. Task 6, transactional catalog reconciliation, is in progress under the [approved database commit-fence boundary](docs/superpowers/plans/2026-09-14-phase-2a-indexed-browser.md#task-6-commit-fence-decision): expired scanner transactions must reject, while deliberate database-session constraint-timing tampering is outside the absolute lease-deadline guarantee. Root permissions and original-file protections are unchanged. Worker scan execution, browsing APIs and new screens are not available yet. The approved scope still includes metadata filters and photo/video thumbnails. The [Phase 1 secure foundation](docs/superpowers/plans/2026-08-31-phase-1-secure-platform-foundation.md) remains complete.
...
Its [implementation plan and task ledger](docs/superpowers/plans/2026-09-14-phase-2a-indexed-browser.md#status-and-task-ledger) define **18 tasks: 5 complete, 13 remaining**. These cover source identity/schema, maintenance authority, safe scanning/recovery, filters/cursor APIs, mobile browsing, real-stack tests, catalog/filesystem/mobile benchmarks, and the operating/acceptance handoff. This is the count for 2A.1 only, not all remaining v1 work. Task 5 passed review at `ddc38b6`: 862 backend tests preceded the boundary fix, then 32 covering reader/preservation tests passed; five actual read-only-container checks passed at `67d4e47`. Its synthetic 50K iterator checks bounded allocation, not physical-scale throughput. All four [Linux CI jobs](https://github.com/troja-gnister/aegis/actions/runs/34889336371) passed at Task 5 checkpoint `136fff0`. Local nested-mount test failures remain [explicitly tracked](docs/superpowers/plans/2026-09-14-phase-2a-indexed-browser.md#task-4-deployment-verification-limitation), separately from Linux results. No package-scale acceptance is claimed yet.
