...
The current focus is **Phase 2A.1 — Indexed-browser implementation**, starting the indexed-drive milestone. Tasks 1–11 are complete: lossless filename handling, protected catalog and scan state, safe read-only indexing/recovery, typed metadata filters, signed cursors, permission-bound queries, private list/details/status/rescan HTTP APIs, and a validated, bounded browser data layer. Scanning preserves logical organization and last-known metadata and retains missing-file records without deleting originals. The [approved database commit-fence boundary](docs/superpowers/plans/2026-09-14-phase-2a-indexed-browser.md#task-6-commit-fence-decision) and [reader-launch/restart contract](docs/superpowers/plans/2026-09-14-phase-2a-indexed-browser.md#task-7-runtime-coordination-decision) remain documented. Task 12 delivers the mobile file-browser screen next; no file-browsing screen or content viewer is available yet. Metadata-filter controls, photo/video thumbnails, and media/document viewers remain within Phase 2. The [Phase 1 secure foundation](docs/superpowers/plans/2026-08-31-phase-1-secure-platform-foundation.md) remains complete.
...
Its [implementation plan and task ledger](docs/superpowers/plans/2026-09-14-phase-2a-indexed-browser.md#status-and-task-ledger) define **18 tasks: 11 complete, 7 remaining**. This is the count for 2A.1 only, not all remaining v1 work. The remaining tasks deliver bounded mobile browsing and filter controls, real-stack journeys, catalog/filesystem/mobile benchmarks, and the operating/acceptance handoff. Task 11's final correction `d58fcb8` passed 89 covering tests, all 96 frontend tests, lint/type/build checks, and independent scoped review. Its private query window retains at most five pages and deduplicates the visible window without retaining historical IDs. Task 10's accepted API passed all 1,346 backend tests; authenticated browsing measured 13 SQL statements ordinarily and 16 when refreshing session activity, within the request budget. Historical verification remains in the plan. The 1M-entry/50K-folder performance gates remain open; bounded synthetic fixtures and query-count checks are not physical-scale acceptance. Local nested-mount failures remain [explicitly tracked](docs/superpowers/plans/2026-09-14-phase-2a-indexed-browser.md#task-4-deployment-verification-limitation), separately from Linux results.

The [frontend CI failure at `557cf71`](https://github.com/troja-gnister/aegis/actions/runs/35016614015) led to a reproduced login/logout session-cache race. Repair `1a0994a` passed 46 frontend tests and independent review. All four [combined-checkpoint CI jobs](https://github.com/troja-gnister/aegis/actions/runs/35639373348) passed at `07704cf`, including the accepted API fixes and frontend repair. [Failure, repair, and verification details](docs/superpowers/plans/2026-09-14-phase-2a-indexed-browser.md#current-checkpoint-ci-limitation) remain tracked. That CI checkpoint predates Task 11; its new combined CI run is pending. Rendered-browser and physical-scale acceptance remain open.
