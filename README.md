...
The current focus is **Phase 2A.1 — Indexed-browser implementation**, starting the indexed-drive milestone. Tasks 1–10 are complete: lossless filename handling, protected catalog and scan state, safe read-only indexing/recovery, typed metadata filters, signed cursors, permission-bound queries, and private list/details/status/rescan HTTP APIs. Scanning preserves logical organization and last-known metadata and retains missing-file records without deleting originals. The [approved database commit-fence boundary](docs/superpowers/plans/2026-09-14-phase-2a-indexed-browser.md#task-6-commit-fence-decision) and [reader-launch/restart contract](docs/superpowers/plans/2026-09-14-phase-2a-indexed-browser.md#task-7-runtime-coordination-decision) remain documented. Task 11's bounded browser data layer is in progress, with the file-browser screen following; no file-browsing screen or content viewer is available yet. Metadata-filter controls, photo/video thumbnails, and media/document viewers remain within Phase 2. The [Phase 1 secure foundation](docs/superpowers/plans/2026-08-31-phase-1-secure-platform-foundation.md) remains complete.
...
Its [implementation plan and task ledger](docs/superpowers/plans/2026-09-14-phase-2a-indexed-browser.md#status-and-task-ledger) define **18 tasks: 10 complete, 8 remaining**. This is the count for 2A.1 only, not all remaining v1 work. The remaining tasks deliver bounded mobile browsing and filter controls, real-stack journeys, catalog/filesystem/mobile benchmarks, and the operating/acceptance handoff. Task 10's final correction `c364394` passed 75 focused tests, 273 covering tests, all 1,346 backend tests, lint/type/migration checks, and independent scoped review. Authenticated browsing measured 13 SQL statements ordinarily and 16 when refreshing session activity, within the request budget. Historical verification remains in the plan. The 1M-entry/50K-folder performance gates remain open; query-count and index-capability checks are not physical-scale acceptance. Local nested-mount failures remain [explicitly tracked](docs/superpowers/plans/2026-09-14-phase-2a-indexed-browser.md#task-4-deployment-verification-limitation), separately from Linux results.

The [frontend CI failure at `557cf71`](https://github.com/troja-gnister/aegis/actions/runs/35016614015) led to a reproduced login/logout session-cache race. Repair `1a0994a` passed 46 frontend tests and independent review. All four [combined-checkpoint CI jobs](https://github.com/troja-gnister/aegis/actions/runs/35639373348) passed at `07704cf`, including the accepted API fixes and frontend repair. [Failure, repair, and verification details](docs/superpowers/plans/2026-09-14-phase-2a-indexed-browser.md#current-checkpoint-ci-limitation) remain tracked; physical-scale and new browser-data gates remain open.
