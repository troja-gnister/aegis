...
The [September 22–24 environment report](verification/linux-podman-prerequisite.md) separates verified toolchain/browser preparation from unverified application compatibility. Reviewed local-engine tooling is committed at `163c02563d95de8e4e3930ae68377e946feef3b8`; full Podman compatibility is not accepted. Its independently reviewed focused PostgreSQL probe passed three tests and exact cleanup. After further corrections passed independent review, the first full backend gate had 1,440 passes and six failures; the deployment gate built the pinned images, then had 359 passes, 71 failures and 10 setup errors. The corrected tooling then passed `make verify`: 1,452 backend tests, 147 frontend tests, static checks and production frontend build. Remaining actual runtime blockers include unsupported tmpfs ownership options and duplicate read-only masks at `/sys/devices/virtual/powercap`. Disabling automatic tmpfs does not resolve the duplicate. No real application browser gate has run. Do not infer general compatibility from the focused probe or resource-free tests.
