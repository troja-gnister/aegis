...
The [September 22–23 environment report](verification/linux-podman-prerequisite.md) separates verified toolchain/browser preparation from unverified application compatibility. The local Podman adapter is still under correction and has not landed. Outstanding work concerns ownership and complete inventories before preparation/cleanup, explicit resource transitions and partial-create recovery, and readable synthetic manifests under rootless UID mapping. No application harness or database has been started on this host. Do not infer runtime safety from the passing resource-free tests.

The isolated WebKit synthetic test container was removed after exact-identity checks; its uniquely owned image/build cache is retained. The temporary rootless API process and socket are absent on resumption. A fresh clone contains neither those resources nor the ignored local prototype/toolchain. Use this public status to retain Tasks 1–12 acceptance while completing the prerequisite; do not treat the uncommitted prototype as a shipped feature.
