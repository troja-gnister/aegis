...
The first work package is [2A.1 indexed browsing](superpowers/specs/2026-09-14-phase-2a-indexed-browser-design.md). Its [implementation plan](superpowers/plans/2026-09-14-phase-2a-indexed-browser.md) defines 18 tasks: four completed and 14 remaining. The filename domain, focused verification commands, protected catalog schema, deployment-bound state and database scheduling/lease boundaries are implemented. Reviewed Task 4 revision `0917747` passed 90 covering backend tests and 42 actual-role/built-artifact tests; the preceding full backend run passed 794 tests at `af01d74`. Deployment installs the fixed scheduling functions with exact role grants, but the physical reader and worker scan loop are not connected yet. Local nested-mount failures and existing platform skips remain documented in the plan; no full-deployment, browsing API/screen or Phase 2 benchmark acceptance is claimed for this checkpoint.
