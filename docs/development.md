...
Task 6 is not accepted: an actual indexer-role test demonstrates that forcing a deferred constraint early permits a subsequent commit after lease expiry. The normal deferred-transaction checks and this deliberate constraint-timing case are different guarantees. The [open design gate](superpowers/plans/2026-09-14-phase-2a-indexed-browser.md#task-6-commit-fence-decision) requires a decision before scanner integration; the failing regression remains intact and Task 6 application changes are not pushed.
