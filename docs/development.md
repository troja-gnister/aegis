...
The first work package is [2A.1 indexed browsing](superpowers/specs/2026-09-14-phase-2a-indexed-browser-design.md). Its [implementation plan](superpowers/plans/2026-09-14-phase-2a-indexed-browser.md) defines 18 tasks: five completed and 13 remaining. The filename domain, focused verification commands, protected catalog schema, deployment-bound state, database scheduling/lease boundaries and bounded read-only reader are implemented. Reviewed Task 5 revision `ddc38b6` passed 32 covering reader/preservation tests after its raw-byte boundary fix; the preceding production code passed 862 backend tests and five actual read-only-container checks at `67d4e47`. Ruff/mypy are clean. The 50K lazy iterator test measures allocation bounds, not physical scanner throughput. All four Linux CI jobs passed at Task 5 documentation checkpoint `136fff0`. The reader is not yet connected to transactional observations or a worker scan loop. Local nested-mount failures and existing platform skips remain separately documented in the plan; no browsing API/screen or Phase 2 benchmark acceptance is claimed for this checkpoint.

Task 6 is in progress, not accepted. On September 15, the user approved stock PostgreSQL with controlled scanner-owned transactions and an explicit exclusion of deliberately altered database-session constraint timing from the absolute lease-deadline guarantee. The [approved boundary and original failing reproduction](superpowers/plans/2026-09-14-phase-2a-indexed-browser.md#task-6-commit-fence-decision) remain documented. Normal expired transactions must still roll back in the database; root/attempt/revocation checks, organization permissions and read-only originals are unchanged. The checkpoint wrappers must own their commit boundary before returning success; complete verification and independent review remain required.
