...
Do not run this harness where those ports belong to another deployment. The current development computer's user preview uses `18080`; leave it running and untouched. Task 14 must first add a validated test-only port override/preflight before local browser-harness execution there. See the [cross-computer handoff](development-handoff.md#task-14-harness-prerequisites).
