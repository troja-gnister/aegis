...
Planning baseline: `842ba2a` on `main`, with unchanged application code from the verified Phase 1 foundation; execution begins from the committed plan at `6169402`. This plan defines **18 tasks: 6 complete, 12 remaining**. Tasks 1–6 passed scoped verification and review; Task 7, supervised scan execution, is in progress with the runtime coordination decision below pending. Task 6 retains the September 15 approved commit-fence boundary below. Task 4's local full-deployment limitation remains documented below, not a green gate. The ledger is authoritative; checkboxes below record the execution recipe and subsequent evidence, not a second task count.
...
| 7 | Supervised scan execution and independent worker liveness | 6 | In progress; runtime coordination decision pending |
...
Implementation is in progress, not accepted. Initial lifecycle and heartbeat checks pass, but the physical launcher is not implemented. Existing startup attestation does not retain a root descriptor; acquiring one before spawning can itself block. A database lease and parent-death signal also do not alone prove that a stalled physical reader has exited before its replacement starts. [Linux parent-death signaling](https://man7.org/linux/man-pages/man2/pr_set_pdeathsig.2const.html) has parent-thread and setup-race constraints; runtime reaping must be tested separately.

The proposed adjustment, awaiting user approval, is an indexer-only local coordination volume outside original roots, with a per-root lock held by both supervisor and reader, plus acquisition of the current attested slot's read-only root descriptor inside the supervised child. The [Linux flock lifetime](https://man7.org/linux/man-pages/man2/flock.2.html) permits the inherited open-file-description lock to remain held while either process retains it. Lock files must not be unlinked, replaced or explicitly unlocked while a child could retain source access. All indexers for this deployment must share the same lock storage; independent volumes or arbitrary multi-host scheduling are not a certified substitute. This proposal adds no write access to originals, database handle in the reader, public port or network service. It is not yet permission to add the volume or change the launch contract.

**Files:** Create `backend/aegis_apps/indexing/{supervisor,runner}.py`, `backend/tests/unit/indexing/test_supervisor.py`, `backend/tests/integration/indexing/test_runner.py`, and `tests/deployment/test_indexer_runtime.py`. Modify `backend/aegis_apps/indexing/reader.py` for the Django-free transport entry point and `backend/aegis_apps/operations/management/commands/run_role.py` at dispatch/lifecycle boundaries; extend `backend/aegis_apps/operations/serializers.py`, the heartbeat SQL in `backend/aegis_apps/common/database_privileges.py`, and existing role/heartbeat tests for the explicit counter keys below. Deployment scope remains unchanged pending the runtime coordination decision.
