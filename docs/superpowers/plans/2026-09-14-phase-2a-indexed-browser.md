...
| 6 | `5654fff` (initial implementation `c200579`) | Full backend: 927 passed before the final EOF-replay and review corrections. Final revision: 62 checkpoint/recovery and 108 actual-role/installed-wheel cases passed; full Ruff/mypy clean (199 files), Django checks/migration drift clean. Actual-login tests cover supported expiry, terminal/replay commits, both root/binding revocation orderings, caller-transaction rejection, malformed batches and preserved metadata/organization. New SQL resources execute from the installed wheel. Independent review and scoped one-file fix review passed; owned databases/image removed. New-checkpoint Linux CI pending. | No physical scanner/pool, HTTP, UI or scale acceptance. Approved deliberate SQL-session timing and exact Unicode-derivation trust limits remain explicit. The 10K request-history query-plan test and maximum-name batching cases are scoped evidence, not the 1M/50K gates. Historical full-run failures, one lost/unknown run and the pre-final full-suite distinction remain recorded below. |
