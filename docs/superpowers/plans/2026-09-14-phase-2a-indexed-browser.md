...
| 9 | Permission-bound indexed keyset queries and details | 3, 8 | Review fixes (`89aebce`) |
...
Independent review found one required correction: early missing-candidate or missing/inactive-root returns can skip captured-principal invalidation, returning not-found instead of the fixed authentication failure after revocation/deactivation. This does not disclose unauthorized data, but the session-invalidating contract must be corrected before acceptance. A minor evidence-attribution finding is corrected above. Task 9 remains in its first fix round; the 85/1,265 results are pre-fix evidence and the completed task count is unchanged.

Correction `6c8b1f3` passed the complete **96-test query/authorization suite**, full Ruff/mypy (216 sources), migration and actual-role checks. Ten new actual-web-login cases first reproduced incorrect not-found responses after revocation/deactivation, then passed with captured-principal checks before terminal rejection; current-principal unknown/unauthorized failures remain identical. Successful read locks retain root-before-user order and the seven-data/11-invocation statement count. The failed-details helper always terminates after checking the user, so it cannot acquire a later root lock. All owned databases were removed; no warnings or pending cleanup remained. A fresh scoped review is pending; no post-fix full-backend or HTTP/scale acceptance is claimed.
