...
A separately scoped runtime adaptation is in progress before Task 13. The [prerequisite report](../../verification/linux-podman-prerequisite.md) records the actual Fedora/x86-64 host, rootless Podman and external Compose provider, locked toolchain, synthetic browser results, runtime failures, and cleanup disposition. The independently reviewed focused disposable PostgreSQL probe passed three tests and exact cleanup. The first full `make verify` attempt passed 1,440 backend tests and failed six observer test doubles; `make verify-compose` built the pinned images, then passed 359 deployment tests with 71 failures and 10 setup errors. Isolated probes reproduced unsupported tmpfs ownership options and a duplicated protected system mount. Reviewed tooling checkpoint `163c02563d95de8e4e3930ae68377e946feef3b8` subsequently passed `make verify` with 1,452 backend and 147 frontend tests, full static checks and frontend build. Pushed checkpoint `db6e107` failed [CI run 35989701646](https://github.com/troja-gnister/aegis/actions/runs/35989701646): backend/frontend passed, Docker deployment/browser journeys failed. Guarded duplicate-mask compatibility at `71da40e` passed independent review, 346 engine/adapter tests, 10 targeted fake cases and the focused actual mount checks, including the corrected observer retry. Its documentation checkpoint `2857a31` passed backend/frontend CI but failed deployment/browser journeys in run `36027572227`; the public result does not identify their causes. Canonical merge correction `520cf1f` passed review and preserves base NNP; isolated Caddy private-tmpfs write/recreation checks passed. The reviewed local PostgreSQL candidate remains uncommitted and failed its bootstrap check; the exit cause is not yet established. PostgreSQL/Caddy application adaptations, full deployment/browser gates and Docker CI failures remain open; no package task or feature status advances. Docker CI support and all original-preservation, network-isolation, mount, init/reaping and resource assertions remain required. The i7 development host is not the calibrated N100 reference.

### Current checkpoint CI limitation

September 24 pause: the [continuation handoff](../../../HANDOFF.md) preserves the local PostgreSQL candidate and pending diagnostic-quoting review finding D1. Its 126 passing resource-free tests do not resolve the actual provider's interpolation failure; the diagnostic has not run in a container. The temporary test API is stopped. Latest inspected pushed checkpoint `f618bae` failed [run 36038914373](https://github.com/troja-gnister/aegis/actions/runs/36038914373) in deployment/browser journeys while backend/frontend passed. These prerequisite failures remain open; Tasks 1–12 stay accepted and Tasks 13–18 stay planned.
...
The failure required repair, deterministic regression coverage, independent review, and fresh CI before advancing the UI. The later checkpoints below record those gates. No passing rerun has been substituted for diagnosis, and no frontend code changed in Task 9. Its accepted query-layer tests remain scoped evidence, not an all-green checkpoint. Development has not updated or accessed the user-owned preview or its originals.
...
Repair `1a0994a` passed independent specification and quality review with no findings. It evaluates current session authority when the query observer decides whether to fetch and settles application logout transitions before test teardown. Three deterministic regressions using production observer options first reproduced the cache write, then passed for pending, confirmed, and unconfirmed sign-out. All 22 authentication tests and 46 frontend tests, lint, TypeScript, and the production build passed on Node 24.20. Existing login/logout assertions remain. This acceptance covers the demonstrated observer-options race, not every possible explicit-refetch or browser scheduling scenario.

Combined checkpoint `07704cf5206fecb98451d7ee7a7b8117a2df9ce2` passed overall CI and every backend, frontend, deployment and browser-journey job in [run 35639373348](https://github.com/troja-gnister/aegis/actions/runs/35639373348). This checkpoint includes both the accepted frontend repair and Task 10 availability corrections. The prerequisite gate is satisfied; Task 11 now owns its additional request/parse ownership, cancellation and bounded private-state regressions. Neither this CI run nor the earlier repair certifies the new browser layer before it is built and tested.
...
| 4 | `0917747` (initial implementation `af01d74`); documentation checkpoint `0aa375e` | Full backend: 794 passed at `af01d74`. After the shared-initialization review fix: 90 covering backend tests and 42 actual-role/fresh installed-wheel tests passed; Ruff and mypy clean (187 files), Django checks and migration drift clean. Actual-login barriers test takeover/revocation and database time after waiting locks. Both manual and periodic paths execute from installed SQL resources. Independent review and scoped fix review passed; owned database/image cleanup verified. All four [Linux CI jobs](https://github.com/troja-gnister/aegis/actions/runs/34885117691) passed at `0aa375e`. | Database boundaries only; no physical reader/pool, scan execution, API, UI or scale acceptance. Request-history composite index is assigned to Task 6. Full local deployment remains failed as detailed below, separately from the successful Linux CI gate. |
| 5 | `ddc38b6` (initial implementation `67d4e47`); documentation checkpoint `136fff0` | Full backend: 862 passed before the raw-byte boundary fix; afterward 32 covering reader/preservation tests passed. Five actual read-only-container tests at `67d4e47` confirmed FD counts 5/5 across 30 success/cancel passes and rejected four same-device descendant binds after proving their presence in the same container. Ruff/mypy clean (192 files); Django checks and migration drift clean. Independent review and scoped fix review passed. All four [Linux CI jobs](https://github.com/troja-gnister/aegis/actions/runs/34889336371) passed at `136fff0`. Owned containers/databases removed. | Reader/protocol only, not a worker loop or transactional catalog update. The 50K lazy iterator proves bounded frames/lookahead/allocation, not physical throughput or 1M acceptance. Device classification and mount-loss unit cases use explicit facades/injection; live unmount recovery remains later work. Exact 4,096/4,097 raw-byte tests cover validation, not a physical maximum-length tree. Task 4's local evidence limitation is retained. |
| 6 | `5654fff` (initial implementation `c200579`); documentation checkpoint `2a8fcd8` | Full backend: 927 passed before the final EOF-replay and review corrections. Final revision: 62 checkpoint/recovery and 108 actual-role/installed-wheel cases passed; full Ruff/mypy clean (199 files), Django checks/migration drift clean. Actual-login tests cover supported expiry, terminal/replay commits, both root/binding revocation orderings, caller-transaction rejection, malformed batches and preserved metadata/organization. New SQL resources execute from the installed wheel. Independent review and scoped one-file fix review passed; owned databases/image removed. All four [Linux CI jobs](https://github.com/troja-gnister/aegis/actions/runs/34980600416) passed at `2a8fcd8`. | No physical scanner/pool, HTTP, UI or scale acceptance. Approved deliberate SQL-session timing and exact Unicode-derivation trust limits remain explicit. The 10K request-history query-plan test and maximum-name batching cases are scoped evidence, not the 1M/50K gates. Historical full-run failures, one lost/unknown run and the pre-final full-suite distinction remain recorded below. |
| 7 | `6022c97` (initial implementation `6774cd2`); reviewed through documentation checkpoint `7fa6e9a` | Final full backend: 1,027 passed; 144 actual-role/installed-wheel checks and 14 final-image Linux runtime cases passed; full Ruff/mypy clean (205 sources), Django checks/migration drift clean. Actual-role cancellation barriers cover idle and between-statement windows, both final commit-admission orderings and independent control progress. Runtime checks cover minute-plus leases, independent roots, interruption/restart, read-only roots, custom coordination ownership, partial-launch failure and exact physical reap. Independent review and scoped fix review passed; all three lifecycle findings addressed. Owned databases, containers, images, coordination volumes and children cleaned after source-preservation checks. All four [Linux CI jobs](https://github.com/troja-gnister/aegis/actions/runs/34999815522) passed at completion checkpoint `e7f3430`. | No HTTP, UI or physical 1M/50K acceptance. Genuine uninterruptible kernel I/O and arbitrary multi-host recovery are not certified. One coordinator and shared local coordination storage are the supported topology. Initial 994/229 results and earlier failed runs remain historical evidence below. |
| 8 | `78b05f1` (initial implementation `fd1737c`); reviewed through documentation checkpoint `5c9d04e` | Final complete filter/cursor files: 153 tests passed; full Ruff/mypy clean (209 sources), Django checks/migration drift clean. Meaningful RED/GREEN regressions cover pre-normalization raw JSON size, exact 8 KiB boundaries, every ASCII/Unicode escape class, and multibyte cursor rejection before decoding. Typed size/date precision, complete cursor keys, context changes, tampering and controlled-clock expiry remain covered. Independent review and scoped fix review passed, both input-bound findings addressed with no new issues. Disposable databases removed. Initial 1,167-test full-backend and 142-test focused evidence is explicitly pre-fix. All four [Linux CI jobs](https://github.com/troja-gnister/aegis/actions/runs/35006055700) passed at completion checkpoint `1ac4eb8`. | Pure metadata-filter and cursor contracts only; SQL translation, current query authorization, HTTP/UI delivery and physical 1M/50K acceptance remain later tasks. No original-file or database authority added. |
| 9 | `6c8b1f3` (initial implementation `89aebce`); reviewed through documentation checkpoint `48799e9` | Final complete query/authorization files: 96 tests passed; full Ruff/mypy clean (216 sources), Django checks/migration drift and actual-role/column parity clean. Ten actual-web regression cases first reproduced early rejection bypassing captured-principal invalidation, then passed; current-principal unknown/unauthorized failures remain identical. Initial 85 focused and 1,265 full-backend results are pre-fix. Independent review and scoped fix review passed; the required authentication finding is addressed and the minor test-role attribution is corrected. Every owned disposable database was removed; preview/operator/original resources remained untouched. | Query layer only; seven data/11 invocation statements excludes HTTP middleware. Task 10 owns the 16-total/8-data full-request budget, response-size, no-store and session-invalidating HTTP gates. SQL index-capability evidence is not natural-planner, deep-chain or physical 1M/50K acceptance. |
| 10 | `c364394` (initial implementation `bccbca0`); completion checkpoint `07704cf` | Final 75 focused, 273 covering and 1,346 full-backend tests passed; full Ruff/mypy clean (221 sources), Django/migration and whitespace checks clean. Ready-state worker/schema regressions and real driver connection/shutdown error regressions first failed, then passed. Unknown/foreign-root equivalence, converted-error request IDs/no-store and actual session-refresh persistence are covered. Independent review and scoped fix review passed with all four findings addressed and no new issues. Owned disposable databases removed. All four [Linux CI jobs](https://github.com/troja-gnister/aegis/actions/runs/35639373348) passed at `07704cf`. | Private metadata/status and authorized rescan APIs only; no browser screen or file-content access. Ordinary HTTP uses 13 statements; session-refresh HTTP uses 16. Physical 1M/50K and rendered private-state gates remain later tasks. |
| 11 | `d58fcb8` (initial implementation `9093744`, first correction `65648e2`); completion checkpoint `dbfed7f` | Final 30 focused, 89 covering and 96 full-frontend tests passed; lint, TypeScript, production build and whitespace checks clean on Node 24.20. Meaningful RED/GREEN regressions cover renewed cursors, valid cross-page overlap, retained duplicate copies after eviction, stale-session execution/error effects, requested details identity, obsolete navigation and generation-owned cleanup failure after form unmount. Independent review and two scoped fix reviews closed all five required findings and the minor query-cache test finding without new issues. All four [Linux CI jobs](https://github.com/troja-gnister/aegis/actions/runs/35666075296) passed at `dbfed7f`. | Validated, bounded private data layer only; no file-browser screen, content access, backend or deployment changes. Query-observer churn and the separate million-step navigation-metadata fixture are not rendered account-switching, physical 1M/50K or source-preservation acceptance. |
| 12 | `a82db75` (initial implementation `913937f`, first correction `c523f98`) | First correction: 96 covering and 129 full-frontend tests passed. Final isolated focus-ownership correction: 29 list/page tests, lint, TypeScript, production build/artifact and whitespace checks passed. Final synthetic Chromium probe passed at 320/390/412 pixels with zero overflow, 44-pixel controls, five retained pages, 14 DOM rows, deep-history/component coverage and browser anchor/keyboard/focus/reduced-motion evidence. Independent review and two scoped correction reviews closed all findings with no new issues. Every owned browser/server was stopped. All four [Linux CI jobs](https://github.com/troja-gnister/aegis/actions/runs/35737649769) passed at `a82db75`, including all 131 frontend tests. | Read-only metadata navigation only, not viewers/downloads, real-stack indexed-file journeys, physical-device testing or 1M/50K acceptance. The local 129-test full run precedes the last isolated correction; final covering, browser and CI evidence is recorded separately below. |
...
All endpoints omit a trailing slash to match existing APIs. Filters are one JSON-valued `filters` query parameter, with `v: 1` and optional `kind`, `type`, `size`, `modified`, `availability`, `prefix`; root and directory are URL/query boundaries, not an arbitrary cross-root filter. The explicit unknown-type token is `__unknown__`; ordinary `unknown` means the real `.unknown` extension. Query, API and browser consumers preserve this distinction. Reject duplicate query parameters. `sort=name|modified|size`, `order=asc|desc`, `limit=1..250`, `parent=<UUID>`, and `cursor=<signed value>` are the other allowlisted fields. Unknown parameters fail with `400` after authentication and root authorization.
...
- [x] **Step 1: Write failing reader and preservation tests.**
...
- [x] **Step 2: Verify red.** Run `uv run --locked python scripts/verify.py backend --test-target backend/tests/unit/indexing/test_reader.py --test-target backend/tests/unit/indexing/test_protocol.py --test-target backend/tests/unit/indexing/test_reader_preservation.py`.

- [x] **Step 3: Implement safe enumeration and framing.**
...
Bound path input to 256 components and 4,096 aggregate raw component bytes. Over-limit input fails safely rather than truncating a path or reporting successful empty enumeration. These are explicit initial operating limits; deeper/longer paths require a tested limit adjustment. The input remains descriptor-relative, never an absolute host path.
...
This task supplies small protocol-only bounded stream-read and sequence/terminal/credit helpers, with at most two outstanding batches and explicit acknowledgment of an outstanding batch. It creates no pipes, processes or supervisor loop. Task 7 owns transport wiring and returning acknowledgment only after commit/discard; test the helpers' length-before-allocation, malformed/truncated-frame, ordering, terminal and credit behavior here.

- [x] **Step 4: Verify green.** Run reader/protocol/preservation tests, Ruff/mypy, and `deployment --test-target tests/deployment/test_index_reader.py`. Confirm reads do not mutate bytes, names, size or modification time; qualify host access-time behavior as the approved spec does.

- [x] **Step 5: Review and commit.** Record FD-leak, IPC-size and actual-mount evidence; commit `feat: add bounded read-only directory enumeration`; push `origin main`.

## Task 6: Atomic observations and safe directory finalization

### Task 6 commit-fence decision

On September 15, 2026, the user explicitly approved continuing with stock PostgreSQL and controlled scanner-owned transactions, excluding deliberately altered database-session constraint timing only from the absolute lease-deadline guarantee. Root permissions, root/epoch/attempt/revocation checks, organization protection and read-only originals remain unchanged. That decision resolved the design pause. Task 6 subsequently passed implementation, verification and independent review at `5654fff` (initial implementation `c200579`), with the revision-specific evidence recorded above.

The original actual-indexer-login reproduction records an observation, forces `aegis_guard_directory_commit` to run with `SET CONSTRAINTS ... IMMEDIATE`, waits past the test lease deadline, then commits. The original assertion required rejection with zero published rows; the observed result was an accepted commit with one published catalog row, after the observer saw zero before commit. Preserve that failed result in the history and retain a clearly named limitation-demonstration test under the approved fault model. Do not hide it with a skip/expected failure or describe it as a fixed bypass. This affects catalog metadata only, not original files.

[PostgreSQL 18 documents](https://www.postgresql.org/docs/18/sql-set-constraints.html) that a caller can force deferred constraint checks early. An initially deferred trigger therefore does not alone provide a non-bypassable commit-time clock check. [Security-definer procedures cannot perform transaction control](https://www.postgresql.org/docs/18/sql-createprocedure.html), so replacing the fixed functions with similarly privileged procedures is not a drop-in solution.

Each checkpoint wrapper must own one top-level transaction, reject caller-owned outer transactions, explicitly defer the named commit guard, and return only after commit succeeds. Task 7 must use these wrappers on its separate bounded mutation connections and acknowledge reader batches only after committed success or explicit discard. The database must still reject ordinary transactions crossing expiry, including terminal transitions, and preserve root/attempt/revocation lock ordering. Deliberate session tampering is evidenced separately. No extension, new authority or SQL-text inspection workaround is authorized. Complete verification and review remain required before Task 7 starts.

Before this approval, uncommitted diagnostic work separately passed 27 checkpoint/recovery cases, 88 actual-role boundary cases and 181 reader/protocol/lease/preservation/index regressions. The early-constraint case failed as described above; complete backend verification and review had not yet run. Work was preserved and owned databases were removed. These historical partial results did not certify the revised wrapper contract.

After approval, wrapper ownership first exposed 13 failing cases, then passed after top-level transaction enforcement. Full static checks found and corrected new-test annotations. One full-backend completion was lost to truncated tool output and remains unknown, not a claimed pass or failure. A subsequently captured full run passed 927 tests. Self-review then reproduced a missing commit check on matching EOF replay (1 failed/4 passed); queuing the guarded work update corrected it. Independent review required one shared source-change delta for coupled version counters. After that final refactor, 62 checkpoint/recovery and 108 role/installed-wheel tests plus full static checks passed; scoped re-review was clean. No unchanged full-suite rerun is claimed for those final narrow corrections.

**Files:** Create `backend/aegis_apps/indexing/checkpoints.py`, `backend/aegis_apps/indexing/sql/{observations,finalize}.sql`, `backend/aegis_apps/catalog/migrations/0002_source_parent_revision.py`, `backend/aegis_apps/indexing/migrations/0003_commit_fence.py` after Task 3's `0002_directory_counter_guard` and the new catalog migration, `backend/tests/integration/indexing/{test_observations,test_finalization,test_recovery}.py`. Modify `backend/aegis_apps/indexing/{models.py,sql/lease.sql,sql/start_run.sql}`, `backend/aegis_apps/catalog/models.py`, `backend/tests/integration/catalog/test_models.py`, `backend/tests/conftest.py`, `backend/aegis_apps/common/database_privileges.py`, `backend/tests/unit/common/test_database_privileges.py`, and `tests/deployment/test_database_roles.py`.
...
Also update `backend/tests/integration/indexing/test_scheduling.py` for its trusted-resource fixture to copy the added observation/finalization SQL files while preserving both deliberate shared-fragment failure assertions. A controlled host-ahead regression in `test_recovery.py` verifies that the shared synthetic scan heartbeat uses database time, matching the unchanged production authority contract; future-heartbeat rejection remains strict. The initial full-backend run reported 4 failures, 912 passes and 10 setup errors. Two missing-resource failures were identified directly; clock skew in that original run remains an inference because timestamps were not retained, separately from the controlled reproduction.

- [x] **Step 1: Write failing preservation and stale-commit tests.**
...
- [x] **Step 2: Verify red.** Run `uv run --locked python scripts/verify.py backend --test-target backend/tests/integration/indexing/test_observations.py --test-target backend/tests/integration/indexing/test_finalization.py --test-target backend/tests/integration/indexing/test_recovery.py`.

- [x] **Step 3: Implement fenced upserts and bounded reconciliation.** The observation function validates the entire batch before writing: fixed keys, at most configured records, serialized payload at most 1 MiB, valid source names/kinds/states, integer domains, consistent null metadata, and a sequence/hash bound to the current attempt. Store the most recent sequence/hash on `DirectoryWork`; exact replay is an idempotent acknowledgement, a changed payload with that sequence rejects, and a gap rejects. Extend Task 3's model with `last_batch_hash` in this migration and the exact column allowlist.

The trusted Python wrapper derives display name, binary name key and type hint through `source_name`, rejecting mismatched domain objects. SQL independently validates raw bytes, canonical binary encodings and bounded derived-field types/lengths, not a second Unicode normalization implementation. The indexer can attest observed metadata only within its validated live maintenance lease; derived strings grant no path, SQL, organization or cross-root authority. Exact Unicode derivation against a compromised indexer login is not an additional database guarantee.

Enriched checkpoint fields can exceed the raw IPC representation's size. Include a small Django-free `indexing/payloads.py` serializer/budget helper and the existing reader/protocol plus their unit/preservation tests in this task's file scope. Reader batching must respect both the complete IPC-frame bound and the enriched checkpoint-payload bound, including envelope and conservative JSONB formatting overhead. Preserve the current IPC shape where practical, and keep database imports out of the reader. Test supported long/escaped/byte-irregular names end to end through checkpoint acceptance. Do not raise the fixed limit, split/resequence batches in the supervisor, or partially publish an oversized batch.

Add nullable `CatalogEntry.source_parent_revision` in catalog migration `0002_source_parent_revision`; the indexing commit-fence migration depends on it. Stamp the current validated source parent's revision atomically with an accepted observation. Existing unknown child markers remain stale until observed, without optimistic backfill. An anchor has no source parent. A missing/inaccessible/non-directory ancestor, unknown marker, or marker/revision mismatch prevents a descendant from being treated as currently available; retain its last-indexed metadata. Verify this database predicate now and carry it into Task 7's source traversal and Task 9's bounded availability resolver. Root-anchor/EOF handling must not immediately invalidate children just observed under the same successful attempt.
...
Use one root/directory coordination boundary for upsert, sealing, finalization, and future event reconciliation. All functions verify binding, root activity/epoch, run/policy epoch, work owner/attempt, current directory revision and database lease time after locking. Add a deferred constraint trigger on each changed `DirectoryWork` record to recheck the live fence at transaction commit in the supported scanner-owned transaction flow, including a retained lease on a terminal transition. Each Python checkpoint wrapper owns a top-level transaction, refuses outer transactions, explicitly defers the named guard and returns only after commit; verify rollback and post-commit visibility through actual role connections. Every batch updates its work record atomically, so this is one commit check per batch, not one per catalog entry. Runtime direct table DML remains denied. Register the internal `aegis_guard_directory_commit()` PL/pgSQL trigger function for ownership/search-path verification, with no runtime/public EXECUTE grant. No caller-controlled session variable alone authorizes a write. Apply the approved narrow constraint-timing limitation above, retaining strict supported-expiry and terminal-transition tests.
...
- [x] **Step 4: Verify green.** Run all new checkpoint/recovery tests and actual-login boundary tests. Exercise finalization over more than 500 missing entries with a newer-epoch survivor and a crash between batches. Run existing operations immutability tests, Ruff and mypy.

- [x] **Step 5: Review and commit.** Record retry, rollback, original-preservation, and commit-fence results; commit `feat: reconcile scan observations with transactional fences`; push `origin main`. Accepted implementation `c200579`, review correction `5654fff`; documentation checkpoint/push follows this ledger update.

## Task 7: Supervised scanner and worker liveness

### Task 7 runtime coordination decision

On September 15, the user explicitly approved an indexer-only coordination volume separate from originals and verified read-only root opening inside the supervised child. This resolved the design pause. Task 7 was subsequently completed at `6022c97` after verification and independent fix review described below; this approval checkpoint is historical. The pre-approval checkpoint passed 53 heartbeat, 36 actual-login parity and 21 lifecycle/ancestry cases; its sender failed because transport was absent.

**Initial verification checkpoint, before review corrections:** implementation `6774cd2` passed the full backend (994 tests in 213.40 seconds), affected deployment suites (229 tests in 310.72 seconds), full Ruff and mypy (205 sources), Django checks and migration-drift detection. Deployment coverage includes two-credit read-only transport; a built image and fresh named volume under UID 501/GID 20; a paced 1,501-entry scan retaining a live lease and fresh heartbeat at 65 seconds while a three-entry root finishes within 15 seconds; six interruption modes; receiver-thread cleanup; replacement exclusion until reap; separate controlled-reaper and actual Docker-init adoption tests; and missing/misowned coordination-mount rejection. The six interruption modes are SIGTERM, exact reader kill, pause/no-progress expiry, database cancellation, and manifest/schema changes. They check no missing-entry finalization and stable observations after stop. All owned resources were removed after preservation checks. The subsequent review findings and corrections are recorded below. These small synthetic fixtures do not certify 1M/50K throughput, memory, arbitrary multi-host failover, or actual uninterruptible kernel I/O.

The first long run failed at the 65-second lease assertion. Diagnostics then established source preparation failed before child creation: a full Root query exceeded the indexer's allowed columns. An actual-role regression reproduced the permission error and passed after narrowing the query; no grant or deadline was relaxed. Separately, a regression caught child exit preceding delivery of the final transport message; its correction passed 52 covering tests. Initial static findings and these failed runs remain historical evidence, not passing gates.

Independent review of `2a8fcd8..34ac41e` identified three material gaps despite those passing tests: cancellation on an idle registered connection could allow a subsequent checkpoint transaction, an unreaped root could retain `scanning` status, and failure after successful process creation could lose supervisor ownership. Fix round 1 revision `6022c97` passed 1,027 full-backend tests, 144 actual-role/installed-wheel checks, 14 current-code Linux runtime tests, and full Ruff/mypy (205 sources), with clean Django/migration checks; scoped specification and quality re-review passed with all three findings addressed and no new issues. Its narrow scope includes the existing checkpoint wrapper and fenced failure SQL, without new grants or callable SQL authority. The initial 994/229 results above precede these corrections. The 14-case runtime gate rebuilt its owned image after the final supervisor edit; the earlier 158-case runtime/role gate is not substituted for that final image result.

Existing startup attestation does not retain a root descriptor; acquiring another before spawning can itself block. A database lease and parent-death signal do not alone prove that a stalled physical reader has exited before its replacement starts. [Linux parent-death signaling](https://man7.org/linux/man-pages/man2/pr_set_pdeathsig.2const.html) has parent-thread and setup-race constraints: arm it in the child, check its expected parent afterward and retain the spawning thread for the child lifetime. Do not use a threaded parent's pre-exec callback.

Use a local named volume `indexer-coordination`, mounted only by indexer at `/srv/aegis/indexer-coordination`, for bounded opaque per-root locks and one deployment-admission lock. Both supervisor and child retain the same open-file-description locks. The [Linux flock lifetime](https://man7.org/linux/man-pages/man2/flock.2.html) permits the inherited lock to survive parent loss while the child retains its descriptor. Never unlink, replace, truncate or explicitly unlock a lock file while another process could retain source access. Missing, unwritable or unsupported coordination storage fails closed. All indexers sharing a deployment/database must share this local volume; independent volumes or arbitrary multi-host scheduling are not certified.

One coordinator owns the configured reader pool per deployment. The deployment-admission lock remains held by reader children after a coordinator crash, preventing replacement pools from accumulating; a stuck orphan can conservatively delay restart admission. Within a live coordinator, per-root slots still isolate stalled roots and service other eligible roots. Add indexer-only runtime init/reaping, actually reap owned children, and test restart exclusion separately from zombie reaping: lock release is not proof of cross-namespace reap. No new network service, port, capability or original-file write is added.

The child opens only the fixed slot from the current attested manifest and matching binding/lease. Verify the descriptor's mount ID, fixed path, observer-derived mount fingerprint and read-only leaf-mount state, then preserve the accepted descriptor-relative reader's identity/stability checks. Manifest filesystem/inode fields are host identities and are not blindly compared with Docker Desktop container values. The reader remains Django-free and receives no database/session handle; only explicit control/result and coordination-lock descriptors are inherited. Image directory ownership supports the configured unprivileged runtime; incompatible operator ownership fails closed, never by changing original permissions.

**Files:** Create `backend/aegis_apps/indexing/{supervisor,runner}.py`, `backend/tests/unit/indexing/test_supervisor.py`, `backend/tests/integration/indexing/test_runner.py`, and `tests/deployment/test_indexer_runtime.py`. Modify `backend/aegis_apps/indexing/reader.py` for the Django-free transport entry point and `backend/aegis_apps/operations/management/commands/run_role.py` at dispatch/lifecycle boundaries; extend `backend/aegis_apps/operations/serializers.py`, the heartbeat SQL in `backend/aegis_apps/common/database_privileges.py`, and existing role/heartbeat tests for the explicit counter keys below. The approved deployment adjustment also includes `compose.yaml`, `docker/backend.Dockerfile`, `tests/deployment/test_compose.py` and `tests/deployment/test_container_boundaries.py`. A focused Django-free `indexing/processes.py` and `backend/tests/unit/indexing/test_processes.py` may isolate physical transport/lock ownership from the coordinator and Django runner.
...
- [x] **Step 1: Write failing lifecycle tests with injected clocks/readers.**
...
- [x] **Step 2: Verify red.** Run `uv run --locked python scripts/verify.py backend --test-target backend/tests/unit/indexing/test_supervisor.py --test-target backend/tests/integration/indexing/test_runner.py`.

- [x] **Step 3: Implement the supervision state machine.**
...
Spawn a reader with an explicit argument vector, closed unrelated descriptors and only the approved control/result/coordination-lock descriptors. Inside the child, open and verify the fixed current-attested-slot read-only root under the runtime contract above, then use the accepted descriptor-relative reader. It imports reader/protocol/domain code, not Django startup; pass no database handle or session and strip credentials/Django configuration from its environment. This remains the indexer trust boundary, not a hostile-processor sandbox. Obtain source components through indexed parent traversal in bounded database batches; hold only the current root-relative component chain and bounded reader packets. If the supported protocol/resource bounds cannot represent a directory, report a degraded unsupported traversal explicitly and do not finalize it missing. Never concatenate a browser-supplied path.

Validate each same-root source ancestry link against Task 6's `source_parent_revision` marker and the current parent's revision/state/kind. Unknown or mismatched markers and cycles cannot authorize physical traversal. Logical ancestry is never a substitute. Preserve the reader's bounded raw-component limits and safe degraded outcome.

Keep one separate supervisor control loop for lease renewal/heartbeat/cancellation, and bounded database mutation execution with separate connections and statement/lock timeouts. Use Task 6's scanner-owned top-level checkpoint transactions; do not wrap them in an outer transaction, force commit guards early, or acknowledge an uncommitted result. These supported transactions retain database expiry checks under the approved narrow session-tampering exclusion. A blocked source read or database statement cannot consume the renewal loop. Use monotonic time for process deadlines, PostgreSQL time for leases. Heartbeats at most every 15 seconds use `current_job_id=None` during scans because `DirectoryWork` is not an `Operation` job; safe metrics contain scan counters, never filenames. Preserve stopping-state monotonicity.
...
The loop admits at most the configured global reader count (default two, maximum four), one per root, honors two-frame credits, and services active roots fairly. Select eligible roots by due time/last service; Task 4's single-root scheduler enforces eligibility and skips busy roots but does not dispatch this global pool. Renewal failure closes the commit gate before any queued batch is applied. Serialize local cancellation with final commit admission immediately at commit, not executor registration or initial validation: cancellation winning before that point forces rollback, including idle-before-checkpoint and between-statement windows. A commit already admitted may resolve after a later stop request; remain stopping until its outcome resolves, admitting no subsequent checkpoint and never claiming it was undone. Keep database lease/root/attempt/manifest checks unchanged and the control loop independent of commit I/O. Test both orderings with actual-role observation/finalization barriers. [PostgreSQL cancellation dispatch](https://www.postgresql.org/docs/18/libpq-cancel.html) alone does not guarantee an abort. Successful EOF transitions to bounded finalization ticks, not a long uninterruptible loop. No-progress timeout applies to observation progress rather than total directory duration; healthy large directories can run longer than 120 seconds.

Wire Task 5's bounded framing and sequence/terminal/credit helpers into the actual transport. Release a batch acknowledgment only after database commit or explicit discard; test this process boundary rather than assuming protocol-only tests prove the wired channel bound.

On stop/timeout, stop accepting observations, close credits, terminate the exact child, wait a bounded grace period, then kill only that child if needed. If it remains unreaped, retain its root slot in `UNREAPED`, publish degraded state and start no replacement for that root. Do not spawn endless retries or claim recovery while a stuck reader persists. On restart, inherited coordination locks exclude overlapping source readers; owned children must be actually reaped and runtime init/reaping tested. Do not infer reaping from lease expiry or a kill request. The scanner never writes within original roots; only the approved coordination-lock inode creation outside originals is added.

- [x] **Step 4: Verify green.** Run lifecycle and runner tests, existing run-role/readiness/heartbeat suites, and `deployment --test-target tests/deployment/test_indexer_runtime.py`. Demonstrate a minute-plus scan retaining a live lease and heartbeat, a second root making progress, and no stale writes after cancellation.

- [x] **Step 5: Review and commit.** Record real-process evidence and resource bounds; commit `feat: supervise recoverable scans without blocking worker liveness`; push `origin main`.
...
- [x] **Step 1: Write failing validation/context tests.**
...
- [x] **Step 2: Verify red.** Run `uv run --locked python scripts/verify.py backend --test-target backend/tests/unit/catalog/test_filters.py --test-target backend/tests/unit/catalog/test_cursors.py`.

- [x] **Step 3: Implement canonical filters and cursor signatures.**
...
- [x] **Step 4: Verify green.** Run filter/cursor unit/property tests, Ruff and mypy. Verify equivalent filters reuse a digest but authorization, page-size or sorting changes invalidate it. Add boundary tests at 899/901 seconds using the signing clock, not sleeps.

- [x] **Step 5: Review and commit.** Record schema/expiry/tampering evidence; commit `feat: define bounded metadata filters and signed browse cursors`; push `origin main`. Accepted implementation `fd1737c`, review correction `78b05f1`; this completion-document checkpoint is pushed with the reviewed code.

### Task 8 input-bound review

Initial implementation `fd1737c` passed 142 final focused/property tests and full Ruff/mypy (209 sources). Its 1,167-test full-backend run followed the last production change, before two test-only property additions covered by the final focused run. Independent review identified two material input-bound gaps: a raw 8,849-byte filter document was accepted after normalization reduced it to 7,394 bytes, and cursor input was counted in Unicode characters rather than bytes. Fix round 1 revision `78b05f1` passed 153 complete filter/cursor tests and full Ruff/mypy (209 sources), with clean Django/migration checks and disposable database cleanup. Regressions first reproduced both defects; exact-budget and JSON-escape parity tests cover the bounded raw-size counter, including a DEL-escape mismatch found and corrected during self-review. Cursor tests prove multibyte input is rejected before decoding. The earlier full-backend result is pre-fix evidence, not a post-fix full-suite claim. Independent scoped specification and quality re-review passed through checkpoint `5c9d04e`, with both findings addressed, no new issues and no unresolved verification gaps. Task 8 is complete. The raw-document encoding and ASCII cursor rules above make the existing byte-bound contract explicit. Query translation, current authorization, HTTP mapping and browser behavior remain subsequent tasks.

## Task 9: Authorized keyset queries and bounded details

**Files:** Create `backend/aegis_apps/catalog/{authorization,queries,serializers}.py`, `backend/aegis_apps/catalog/migrations/0003_browse_indexes.py` after Task 6's `0002_source_parent_revision`, `backend/tests/integration/catalog/{test_authorization,test_queries,test_query_bounds}.py`. Modify the catalog model/privilege column maps if stored/generated ordering columns are added; extend `backend/tests/conftest.py` with `browse_fixture`.

**Interfaces:** Produces `browse_context(user: User, root_id: UUID, namespace: str) -> ContextManager[BrowseContext]`, `directory_page(context: BrowseContext, parent_id: UUID | None, filters: FileFilter, sort: str, order: str, limit: int, cursor: str | None) -> dict[str, object]`, and `entry_details(user: User, entry_id: UUID, namespace: str) -> dict[str, object]`. `BrowseContext` holds the locked active root/user, epochs, and namespace. `CatalogNotFound`/`CatalogNotReady` live in `authorization.py`; `CursorRestartRequired` lives in `cursors.py`. Add `CatalogUnavailable(RuntimeError)` and `CatalogAuthenticationRequired(RuntimeError)` in `authorization.py` with no-argument constructors and fixed `catalog_unavailable` / `authentication_required` messages. All are fixed safe exceptions.

- [x] **Step 1: Write failing order, authorization, and query-budget tests.**
...
- [x] **Step 2: Verify red.** Run `uv run --locked python scripts/verify.py backend --test-target backend/tests/integration/catalog/test_authorization.py --test-target backend/tests/integration/catalog/test_queries.py --test-target backend/tests/integration/catalog/test_query_bounds.py`.

- [x] **Step 3: Implement shared read authorization, indexes and lexicographic predicates.**
...
Compare the locked user's epoch with the request/session epoch captured before authorization; a mid-request change rejects the session rather than minting a new cursor under an old browser namespace. A changed binding must likewise reject an old web process's configured manifest. Add two-connection tests for both races. Raise `CatalogAuthenticationRequired` for this stale captured authentication state, retaining the existing `authentication_required` wire vocabulary. Task 10 maps it explicitly to the foundation 401/session-invalidation response; the query layer does not depend on DRF exception defaults or invent a `session_expired` response type. Map known database statement/lock timeouts to `CatalogUnavailable`, not unrelated programming errors.
...
- [x] **Step 4: Verify green.** Run query/authorization tests with all six sorts, next/previous traversal, filters and revocation barriers; inspect captured SQL. Run actual-role SELECT tests, migration drift, Ruff and mypy.

- [x] **Step 5: Review and commit.** Record query count, authorization, precision and ordering evidence; commit `feat: query indexed directories with authorized keysets`; push `origin main`.

### Task 9 implementation verification

Initial implementation `89aebce` passed **85 focused query/authorization tests and 1,265 full-backend tests**, full Ruff, mypy (216 sources), Django checks and migration-drift detection. Actual web-login tests cover shared readers, lock timeout, ordinary/mixed-visibility pages and details, with catalog mutation authority denied. Separate real two-connection tests cover foundation grant revocation, captured-user epoch changes, binding replacement and candidate-lookup revocation using the privileged fixture reader; the binding mutator uses an actual migrator login. The six sort directions preserve complete keysets and bounded live/missing branches; measured direct browsing uses seven data statements and 11 statements including transaction/timeouts. This excludes HTTP middleware, whose 16-total gate remains Task 10.

Self-review regressions corrected source-recursion seed aliasing, logical ancestor invalidation, consistent source-name projection, explicit nearest-label ordering and valid terminal-anchor checks. Details return at most 64 nearest labels from 65 bounded candidates; invalid ancestors beyond that cap still affect availability. Six `EXPLAIN ANALYZE` capability tests select the matching indexes after fixture statistics are analyzed with sequential scans disabled; they are not scale or natural-planner performance certification. The initial fixture/static failures and earlier 60/84-test intermediate runs are superseded by the initial implementation's final gates. All owned memory-only verification databases were removed; no preview, original or operator resource was touched.

Independent review found one required correction: early missing-candidate or missing/inactive-root returns could skip captured-principal invalidation, returning not-found instead of the fixed authentication failure after revocation/deactivation. This did not disclose unauthorized data, but violated the session-invalidating contract. The first fix round addressed it, and a minor evidence-attribution finding is corrected above. The 85/1,265 results remain pre-fix evidence.

Correction `6c8b1f3` passed the complete **96-test query/authorization suite**, full Ruff/mypy (216 sources), migration and actual-role checks. Ten new actual-web-login cases first reproduced incorrect not-found responses after revocation/deactivation, then passed with captured-principal checks before terminal rejection; current-principal unknown/unauthorized failures remain identical. Successful read locks retain root-before-user order and the seven-data/11-invocation statement count. The failed-details helper always terminates after checking the user, so it cannot acquire a later root lock. All owned databases were removed; no warnings or pending cleanup remained. Fresh scoped review through `48799e9` passed specification and quality, confirmed the required fix and corrected attribution, and found no new issues or out-of-scope observations. Task 9 is accepted; no post-fix full-backend or HTTP/scale acceptance is claimed.
...
- [x] **Step 5: Review and commit.** Record HTTP status/cache/CSRF/permission evidence; commit `feat: expose private catalog and scan status APIs`; push `origin main`.

### Task 10 implementation verification

Initial implementation `bccbca0` passed **52 focused API/status tests, 223 covering tests, and 1,328 full-backend tests**. Full Ruff, mypy (221 sources), Django checks, migration drift, and whitespace checks passed. The covering run includes existing authentication/session, root/operation, query, rescan, and database-privilege cases. Independent review found two required corrections; these initial results remain pre-fix evidence, superseded by the final verification below.
...
Review corrections `c364394` make worker/schema availability govern a persisted `ready` status as well as active scans. Missing, stale, wrong-release/schema/manifest heartbeats and pending schema now fail closed; healthy ready state needs no active run. Narrowly recognized Django/psycopg connection failures and PostgreSQL shutdown/availability errors return the fixed 503 problem after authentication. Classification uses exception types/SQLSTATEs, never message text; programming, integrity, data and unrelated deadlock failures remain generic 500s. The same round compares a real ungranted root with an unknown root and extends common converted-error request-ID/no-store assertions across all four route families.

Final verification passed **75 focused tests, 273 covering tests and all 1,346 backend tests**, with full Ruff/mypy (221 sources), system/migration and whitespace checks clean. The full run followed the last production/test edit. Activity-refresh browsing measured **16 SQL statements**: one session-store read, one session update, one Django principal read, seven catalog/authorization data statements, four catalog transaction/timeout statements and two session-write transaction statements. The durable regression requires real session persistence and enforces the unchanged 16-total/eight-principal-catalog ceilings. Every owned disposable database was removed; no preview, operator or original resource was accessed.

Fresh scoped review passed specification and quality: both required findings and both minor coverage corrections are addressed, with no new or out-of-scope findings. Task 10 is accepted. All four [combined-checkpoint CI jobs](https://github.com/troja-gnister/aegis/actions/runs/35639373348) passed at exact `07704cf`; rendered browser cleanup, physical-scale performance and content delivery remain their later gates.
...
Independent review returned five required corrections: refreshing pages with newly signed opaque cursors, enforcing query ownership before execution/publication/error side effects, matching details to the requested entry ID, making obsolete navigation owners inert, and reporting cleanup failures while finishing teardown safely. A separate minor finding requested behavioral query-map churn coverage; the million-step fixture bounds navigation metadata only. Acceptance required the regression tests and scoped reviews recorded below. The user-owned preview, originals and operator resources remained outside this work.

### Task 11 correction verification

Correction `65648e2` passed **85 covering tests across eight files and all 92 frontend tests across ten files**, lint, TypeScript, the production build and whitespace checks on Node 24.20. Regression cases first reproduced wrong-entry details, writable obsolete navigation, stale-account query execution/error effects, renewed-cursor refetch failure, swallowed cleanup errors and premature sign-out completion. Intermediate failing cleanup tests exposed unhandled sanitized errors; covering and final output was clean.

The correction checks query authority at execution and after API settlement, makes obsolete navigation owners inert, validates the requested details ID, and revises current-window pagination handling. Cleanup failures now finish the remaining teardown and use a sanitized, generation-owned failure outcome that keeps private content and sign-in closed; ordinary server-unconfirmed sign-out remains separate. Logout waits for both the server and browser-cleanup outcomes. A real query-observer fixture exercises 100 filter/namespace changes with at most one active query; this does not extend the separate navigation-only million-step fixture into a physical-scale claim.

The first scoped re-review confirmed query ownership, requested details identity, obsolete navigation ownership and bounded query-cache coverage. Two corrections remained: legitimate cross-page overlap must deduplicate instead of returning 502, and a cleanup failure must close its still-current login transition even after the originating form unmounts. The results above are pre-second-fix evidence, not final acceptance.

Final correction `d58fcb8` passed **30 focused tests, 89 covering tests across eight files and all 96 frontend tests across ten files**, lint, TypeScript, production build and whitespace checks on Node 24.20. Five meaningful failures first reproduced append/prepend/refetch overlap errors, loss of a later duplicate after earlier-page eviction, and dropped cleanup failure after form unmount. The focused suite then passed all 30 cases. The query cache keeps at most five raw pages; a bounded observer `select` projection deduplicates their current contents while preserving page metadata and page parameters. Consumers use the selected observer data, not manually flattened raw cache pages. Still-current login cleanup failures close private access independently of component mount state; obsolete transitions cannot close a newer session.

Final independent scoped review closed both remaining findings without new issues. **Task 11 is complete: 11 tasks complete, 7 remaining.** All four [combined-checkpoint CI jobs](https://github.com/troja-gnister/aegis/actions/runs/35666075296) passed at `dbfed7f`. No file-browser screen, source access, preview refresh, physical-scale or rendered-browser acceptance is included in this data-layer slice.

## Task 12: Virtualized mobile file navigation

**Files:** Create `frontend/src/features/files/{FilesPage,VirtualFileList,FileRow}.tsx`, `frontend/src/features/files/files.css`, `frontend/src/features/files/test-fixtures.ts`, and `frontend/src/features/files/{FilesPage,VirtualFileList}.test.tsx`. Modify `frontend/src/features/roots/{RootCard.tsx,RootListPage.test.tsx}`, `frontend/src/app/router.tsx`, `frontend/package.json`, `frontend/package-lock.json`, the existing root-card link/retired explanation styles in `frontend/src/styles/global.css`, the obsolete root-button assertion in `frontend/e2e/phase1.spec.ts`, and `frontend/src/features/auth/{AuthBoundary.tsx,AuthBoundary.test.tsx}` for existing-session revalidation on browser back/forward navigation.

**Sequencing clarification:** The foundation journey's root-button explanation assertion moves forward from Task 14 because this task replaces that button with a real navigation link. Preserve existing authentication, isolation, history, viewport and reduced-motion checks; real indexing fixtures and expanded file-browser journeys remain Task 14.

**History-state boundary:** Filter, sort, cursor and anchor data stay in bounded session-owned memory, never `location.state`/browser history storage. The persistent application route must revalidate the existing session on back/forward navigation and withhold private rows until current validation settles, while preserving bounded restoration metadata. The auth-boundary file scope above covers that integration; it does not introduce another authentication mechanism.
...
- [x] **Step 1: Write failing navigation and bounded-render tests.**
...
- [x] **Step 4: Verify green.** Run all file/root component tests, lint/types/build, and a local Playwright probe at 320/390/412-pixel widths using synthetic API fixtures. Count DOM rows and retained pages through repeated eviction; screenshot only synthetic names with the existing masking rules.

- [x] **Step 5: Review and commit.** Record keyboard, viewport and bounded-render evidence; commit `feat: add virtualized dark mobile file browsing`; push `origin main`.

### Task 12 implementation verification

Initial implementation `913937f` passed **41 focused, 101 covering, and all 116 frontend tests**, with clean lint, TypeScript, production build, and whitespace checks on Node 24.20. The production bundle excluded test-only virtualizer instrumentation and synthetic fixtures. The scoped code adds authorized opaque-ID routes, root links, a five-page virtualized mobile list, bounded inline metadata selection, and memory-only navigation. Back/forward session validation withholds private content while retaining bounded restoration metadata. There is no file-content access or preview deployment change.

A synthetic Chromium probe passed at 320/390/412 CSS-pixel widths: zero horizontal overflow, minimum 44-pixel controls, 14 initial DOM rows at each width, and 22 rows after repeated eviction. The presented window held five pages; loading through page 11 and observing previous cursor 6 provided behavioral evidence of the retained raw window. Keyboard activation, a retained visible anchor, and reduced motion passed. The first probe failed because its forced top-edge scroll legitimately loaded a previous page, invalidating the next expected cursor; a script-only correction resolved that mismatch. Browser contexts and the separately owned loopback server were stopped. These are synthetic browser measurements, not real-stack, WebKit, physical-device, or 1M/50K acceptance.

Independent review found four required corrections: POP cleanup completion must survive route unmount, history must save the page containing the actual visible anchor, page eviction must not steal focus from paging controls, and a missing anchor/focused row must choose the nearest surviving neighbor in the previous ordering. The initial focus assertion accepted the wrong shifted-index fallback; its passing result does not certify that requirement. The results above are pre-fix evidence, separate from correction verification below.

### Task 12 correction verification

Correction `c523f98` passed **96 covering tests and all 129 frontend tests**, with clean lint, TypeScript, production build, production-artifact and whitespace checks on Node 24.20. Meaningful failing regressions first reproduced delayed POP cleanup remaining unfinished after unmount, restoration from the wrong retained page, stolen paging/outside-control focus, and shifted-index neighbors in both eviction directions. The deep-history test uses the real authenticated routes, proves the inactive query was evicted, withholds filenames during session validation, and restores the cursor belonging to the visible anchor. Obsolete cleanup cannot close a newer session.

Browser diagnostics additionally exposed native focus loss from disabling a loading button, unintended backward fetching after programmatic scroll restoration, and an offscreen focus attempt occurring before its control mounted. Targeted regressions reproduced each before correction. Loading controls now preserve focus with ARIA state and duplicate-request guards; restoration suppresses its matching automatic-paging event without disabling later user paging; one bounded pending focus ID waits for the replacement control. One failed rerun served stale development-server transforms, confirmed by comparing served code with disk; that run is not corrected-source evidence. Subsequent servers were restarted and their served code verified before testing.

The final synthetic Chromium probe passed at **320/390/412 CSS pixels**, with zero horizontal overflow, minimum 44-pixel controls, and 14 rendered rows at each width and after repeated eviction. It retained five presented pages; forward paging through page 11 followed by previous cursor 6 gives behavioral evidence of the raw pages 7–11 window. Native keyboard selection/paging, the surviving visible anchor, loading-button focus, nearest offscreen row focus (0160 to 0200), its announcement, no restoration-triggered extra request, and reduced motion passed. Actual-library component tests separately check measurement/key-cache bounds. All owned browser contexts and loopback servers were stopped, with port closure confirmed. No originals, operator resources, or preview deployment were accessed. Real-stack Chromium/WebKit journeys, physical devices, and the 1M/50K gates remain later work.

The first scoped re-review closed the four original findings but identified one new focus-ownership race: deferred fallback could override a paging or outside control that acquired focus while the replacement was pending. Correction `a82db75` first reproduced both cases, then passed all **29 list/page tests**, lint, TypeScript, production build/artifact and whitespace checks. Fulfillment now checks the captured focus owner and cancels stale fallback; the announcement follows successful focus only. The full 129-test result above belongs to the preceding revision, not this last isolated correction.

The final synthetic mobile probe repeated every width, page-window, anchor, focus, keyboard and reduced-motion assertion against verified current served code and passed. A second independent scoped review closed the remaining finding with no new or out-of-scope issues. Exact checkpoint `a82db75d9949170e1d22acfed0f08050f067edf2` then passed overall [CI run 35737649769](https://github.com/troja-gnister/aegis/actions/runs/35737649769) and all four backend, frontend, deployment and browser-journey jobs; the frontend job passed all 131 tests across 12 files. **Task 12 is complete: 12 tasks complete, 6 remaining.** The implementation and both corrections are pushed on `main`; work pauses before Task 13, with no preview refresh or original-file changes.
...
Seed `source_parent_revision` consistently with each intended-current parent, preserving the Task 6 ancestry contract; anchors have no parent marker. Include explicitly labeled unknown/mismatched-marker cases as stale-availability fixtures. Verify expected nonempty page membership and effective availability, so an invalid all-stale dataset cannot produce a misleading fast benchmark. This is synthetic catalog setup only, not an optimistic backfill of operator data.
