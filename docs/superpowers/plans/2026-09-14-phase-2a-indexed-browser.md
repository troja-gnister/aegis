...
| 7 | Supervised scan execution and independent worker liveness | 6 | In progress; runtime coordination adjustment approved |
...
**Implementation checkpoint, not task acceptance:** after approval, real two-credit transport and the read-only Linux reader passed focused checks. A built image and fresh named volume passed under UID 501/GID 20. The paced 1,501-entry scan passed in 104.50 seconds, retaining a live lease and fresh heartbeat at 65 seconds while a three-entry root completed within 15 seconds. Six real interruption cases passed in 84.96 seconds: SIGTERM, exact reader kill, pause/no-progress expiry, database cancellation, and manifest/schema changes. They checked no missing-entry finalization, stable observations after stop, and owned-child reaping. Restart/init evidence, final static/regression checks and independent review remain required. These small synthetic fixtures do not certify 1M/50K throughput, memory, or actual uninterruptible kernel I/O.
