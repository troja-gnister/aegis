...
As of this implementation checkpoint: seven milestones defined, zero accepted. The [2A.1 implementation plan](../plans/2026-09-14-phase-2a-indexed-browser.md#status-and-task-ledger) defines **18 tasks: 12 complete, 6 remaining**. Filename handling, focused verification, protected catalog, deployment-bound state, scheduling/leases, the bounded read-only reader and transactional reconciliation are implemented and reviewed. Task 7's supervised execution, indexer-only coordination storage and read-only child-side root opening passed verification and independent review. Task 8's typed filters and signed cursors, including input-byte-limit corrections, passed focused verification and independent scoped review. Task 9's permission-bound indexed queries and bounded details are accepted after authentication-precedence corrections passed covering verification and independent scoped review. Task 10's private catalog/status/rescan HTTP APIs are accepted after worker-health and outage-mapping corrections passed the full backend suite and independent scoped review. Tasks 11–12's bounded browser data and virtualized mobile navigation passed verification and independent review, including history, cleanup and focus corrections. A separate [Linux/Podman prerequisite](../../verification/linux-podman-prerequisite.md) has a passing focused disposable PostgreSQL probe, but its first full backend and deployment gates failed. Reviewed tooling checkpoint `163c025` subsequently passed `make verify` with 1,452 backend and 147 frontend tests. Guarded duplicate-mask compatibility at `71da40e` passed independent review and focused actual mount/nested-mount checks. Canonical merge correction `520cf1f` preserves base NNP and passed review; isolated Caddy private-tmpfs write/recreation checks passed. The local PostgreSQL candidate failed its bootstrap check; the exit cause is not yet established. PostgreSQL/Caddy application adaptations and full deployment/application browser compatibility remain open before Task 13. Pushed checkpoints `db6e107` and `2857a31` failed Docker CI deployment/browser journeys while backend/frontend passed; these failures also remain prerequisites. Filter/details/scan-state interactions, real-stack indexed-file journeys and scale delivery remain open. The [development handoff](../../development-handoff.md) records the resume boundary. Under the September 15 approved database boundary, controlled scanner transactions retain database expiry checks; deliberately altered SQL-session constraint timing is excluded only from the absolute lease-deadline guarantee. Root permissions and original-file protections remain unchanged. Local deployment evidence limitations remain recorded in the plan, with no package acceptance. Subsequent package task counts have not yet been assigned; neither seven milestones nor 18 first-package tasks is the full remaining rewrite task count.

| Milestone | Design/implementation state | Accepted packages | Acceptance evidence |
| --- | --- | --- | --- |
| 2A | [2A.1 specification](2026-09-14-phase-2a-indexed-browser-design.md) approved; [18-task plan](../plans/2026-09-14-phase-2a-indexed-browser.md) has indexed queries, HTTP endpoints, bounded browser data and mobile navigation accepted; Linux/Podman prerequisite verification precedes filter/details/scan-state controls | 0 | No package acceptance yet |
