...
#define FAIL2BAN_JAIL_D     "/etc/fail2ban/jail.d"
#define FAIL2BAN_CONV_DIR   "/etc/aegis/rules.d/fail2ban"

/* Copy any *.conf files from the convention directory to /etc/fail2ban/jail.d/ */
static void apply_convention_dir_fail2ban(aegis_executor_t *exec, aegis_result_t *res) {
    const char *ls_argv[] = {"ls", FAIL2BAN_CONV_DIR, NULL};
    ...
    while (tok) {
        ...
        if (flen > 5 && strcmp(tok + flen - 5, ".conf") == 0) {
            char src_path[512];
            char dst_path[512];
            snprintf(src_path, sizeof(src_path), "%s/%s", FAIL2BAN_CONV_DIR, tok);
...
aegis_result_t aegis_fail2ban_apply(const void *config, aegis_executor_t *exec) {
    ...
    /* Apply any custom jail overrides from convention directory */
    apply_convention_dir_fail2ban(exec, &res);
